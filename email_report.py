...
from shensha import get_chong_zodiac, get_ji_shi
from huangli import get_yi_ji, get_jieqi_detail
...
.pillar-row { display:flex; gap:16px; justify-content:center; flex-wrap:wrap; margin:8px 0; }
.pillar { background:#f8f9ff; border-radius:8px; padding:12px 18px; text-align:center;
          min-width:80px; }
.pillar .ganzhi { font-size:20px; font-weight:700; color:#2d5016; }
.pillar .nayin { font-size:11px; color:#636e72; margin-top:4px; }
.wuxing-bar { margin:8px 0; display:flex; align-items:center; gap:8px; }
...
def render_plain(report: dict) -> str:
    ...
    lines = [
        ...
        "=" * 40,
        f"📅 今日干支: {report['year_ganzhi']}年 {report['month_ganzhi']}月 {report['day_ganzhi']}日",
        f"   纳音: 年柱{report['nayin_year']} 月柱{report['nayin_month']} 日柱{report['nayin_day']}",
        "",
        "🔥 五行旺衰:",
    ]
...
<div class="header">
  ...
  <p>仅供参考，不构成建议</p>
    ...
    # -- 干支区 --
    parts.append('<div class="section"><h2>📅 今日干支</h2>')
    ...
    for label, gz, nayin in [
        ...
        parts.append(f'<div class="pillar"><div style="font-size:11px;color:#636e72">{label}</div>'
    ...
    for wx in ["木","火","土","金","水"]:
        ...
        s = data["score"]
        pct = min(100, max(0, (s + 2) * 10))  # map [-2, 8] -> [0%, 100%]
